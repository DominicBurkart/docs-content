...
In this how-to guide you learn how to create and push a container image to the Scaleway [Container Registry](/compute/container-registry/concepts/#container-registry) and how to use it on [Kubernetes Kapsule](/compute/kubernetes/concepts#kubernetes-kapsule/).
...
The generated Docker images are stored in a private Docker registry using the Scaleway [Container Registry](/compute/container-registry/concepts/#container-registry) product. 
...
4. Check that the image has been added to the local Docker installation by running the `docker images` command. Your newly created container image will be displayed in the list of available images:
    ...
    ```
5. [Log into your Container Registry](/compute/container-registry/how-to/connect-docker-cli). The endpoint (for example: `rg.fr-par.scw.cloud/myregistry`) of your Container Registry is available from the [Scaleway Console](https://console.scaleway.com/registry/namespaces) and depends on your configuration:
    ```bash
