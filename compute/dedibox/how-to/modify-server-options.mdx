---
meta:
  title: How to modify your Dedibox server options
  description: This page explains how to modify your Dedibox server options
content:
  h1: How to modify your Dedibox server options
  paragraph: This page explains how to modify your Dedibox server options
tags: dedibox link scaleway
dates:
  validation: 2022-03-17
  posted: 2022-04-12
---

<Message type="requirement">
  - You have an account and are logged into the [Scaleway console](https://console.scaleway.com) 
  - You have performed the [linking process](/compute/dedibox/how-to/link-dedibox-account/) between Dedibox and Scaleway
  - You have [ordered](/compute/dedibox/how-to/order-dedibox) a Dedibox server
</Message>

1. Click **Dedibox** in the **Compute** section of the [Scaleway Console](https://console.scaleway.com) side menu. The Dedibox dashboard displays.
2. Choose the server you want to configure from the list. Click its name to display that server's information page.
3. Scroll down to the **Server options** section and click **Modify options**.
    A list of modifiable options for your server displays. 
4. Modify the service level for your Dedibox as required. Based on the service level you can customize your server with additional options, such as storage or DDoS protection.
    <Lightbox src="scaleway-modify-options.webp" alt="" />
5. A summary of your options and configuration modifications displays. Click **Save changes** to validate.
    <Message type="important">
      If you order a paid option, the price for the option or customization will be added to your next monthly invoice.
    </Message>


<Navigation title="See Also">
  <PreviousButton to="/compute/dedibox/how-to/configure-ipv6-dedibox">How to configure IPv6</PreviousButton>
  <NextButton to="/compute/dedibox/how-to/use-rescue-mode">How to use rescue mode</NextButton>
</Navigation>