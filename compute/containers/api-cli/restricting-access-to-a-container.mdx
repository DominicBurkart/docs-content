...
```sh
curl --location --request GET 'https://api.scaleway.com/containers/v1beta1/regions/fr-par/issue-jwt?namespace_id={your_namespace_id}&expires_at=2022-06-22T00%3A00%3A00Z' --header 'X-Auth-Token: {your_scaleway_API_key'}
```
...
2. Enter the API URL. 
    * For namespaces:
    ```
    https://api.scaleway.com/containers/v1beta1/regions/fr-par/issue-jwt?namespace_id={your_namespace_id}&expires_at={expiration_date}
    ```
    * For Containers:
    ```
    https://api.scaleway.com/containers/v1beta1/regions/fr-par/issue-jwt?container_id={your_container_id}&expires_at={expiration_date}
    ```
