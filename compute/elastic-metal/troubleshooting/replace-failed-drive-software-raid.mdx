...
1. Once the failed disk is replaced, copy the partition table of the source disk to the new disk:
    ...
    The `sfdisk` command above replaces the entire partition table on the new disk with the one of the source disk. Modify the command if you require to preserve other partition information on the disk.
