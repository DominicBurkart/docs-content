---
meta:
  title: How to deploy WordPress with LEMP on Ubuntu Bionic Beaver
...
content:
  h1: How to deploy WordPress with LEMP on Ubuntu Bionic Beaver
  paragraph: In this tutorial, you will learn how to install WordPress on a freshly created Ubuntu Bionic Beaver Instance with LEMP.
...
In this tutorial, we will learn how to install WordPress on a freshly created Ubuntu Bionic Beaver instance with LEMP (Linux + Nginx - pronounced "engine x" + MySQL + PHP). Nginx is an HTTP server that, compared to Apache, uses less resources and delivers pages much faster, especially static files.
...
- You have [secured your LEMP stack with TLS/SSL](/tutorials/installation-lemp-ubuntu-focal/#configuring-ssl-with-let's-encrypt)
</Message>

## Installing the stack

As a tutorial on LEMP is already available, you can follow the procedure at [Installing LEMP on Ubuntu Bionic Beaver.](/tutorials/installation-lemp-ubuntu-focal/)
...
You will now need to configure Nginx to deliver traffic to the future WordPress installation. In the [LEMP tutorial](/tutorials/installation-lemp-ubuntu-bionic/) we created a server block for `example.com`, serving the default Nginx page from our domain `example.com`. Here, we will create a server block `wordpress` which will serve a WordPress site from our subdomain `blog.example.com`.
...
1. Select your language.
  You are asked to enter your database information. Before doing so, go back to the terminal to complete the next steps.
