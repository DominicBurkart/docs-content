...
[Gitea](https://www.gitea.io/) is a self-hosted Git service. It provides a lightweight code hosting solution written in Go and published under the MIT license. The tool is a community-managed fork of [Gogs](https://gogs.io/).

[Git](https://en.wikipedia.org/wiki/Git) itself is a distributed version control system for tracking changes in files.
The tool was initially invented by [Linus Torvalds](https://en.wikipedia.org/wiki/Linus_Torvalds) in 2005 to manage the development of the Linux kernel among other developers contributing to its code-base.

Unlike many other client-server systems, every Git directory on every computer contains the whole repository with a complete history and version tracking, making it independent of network access or a central server.

Git was initially developed for software development, but it can track changes in any file.
