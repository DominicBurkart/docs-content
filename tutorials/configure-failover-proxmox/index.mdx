...
8. Configure the network interface. Set the **bridge** to `vmbr0`, the model to `VirtIO (paravirtualized)` and configure the [virtual MAC address](/dedibox-network/ip-failover/concepts#virtual-mac-address) assigned to the failover IP of the virtual machine. The static [network configuration](/compute/elastic-metal/how-to/configure-flexible-ip/) is done in a later step, during the installation of the operating system.  <Lightbox src="scaleway-proxmox_kvm_cpu_05.webp" width="500" alt="" />
