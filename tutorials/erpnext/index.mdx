---
meta:
  title: How to deploy ERPNext 12 on Ubuntu 18.04 LTS (Bionic Beaver)
  description: Discover how to deploy the ERPNext 12 application on a Scaleway Dedibox dedicated server
content:
  h1: How to deploy ERPNext 12 on Ubuntu 18.04 LTS (Bionic Beaver)
...
2. Create a new user. Some of the commands we will be running later in this tutorial should not be carried out as root. Here, we call the new user `mariecurie`, but you could call them `erpnext` or whatever you like. Enter a password for them when prompted. You can leave the user information at default values if you wish.
  ...
  ```
3. Give the new user sudo priveleges:
  ```
...
1. Open a web browser on your local computer and point it to `https://your_erpnext_domain/`. The login page displays. Enter the user name **Administrator** and the password defined during the setup. Then click **Login**:
