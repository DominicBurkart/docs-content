...
<Message type="requirement">
  ...
  - You have [created](/compute/instances/how-to/create-an-instance/) and [connected to](/compute/instances/how-to/connect-to-instance/) a Scaleway Instance with at least 2GB RAM
  - You have configured a domain name to point toward the IP address of your Instance ([A or AAAA record](/network/dns-cloud/concepts#resource-records))
...
1. Update the apt package manager and upgrade the software installed on the server to ensure your system is up to date.
  ...
  ```
2. Magento relies on a webserver to deliver content. Install the Apache2 web server using the apt package manager with the following command:
...
1. Elasticserach uses a secure HTTPS transaction for its repositories. If not installed yet, install the `apt-transport-https` package use the `apt` package manager:
...
By default plain HTTP connections are not encrypted and data is transmitted in clear text over the Internet. To secure the connection between your users and the e-commerce site, a TLS/SSL certificate is highly recommended. We will therefore request a free TLS certificate from [Let's Encrypt](https://letsencrypt.org/). We use [certbot](https://certbot.eff.org/) to obtain and renew the certificate automatically for us.
...
2. Disable 2FA with the following command:
  ...
  ```
3. Log into the admin using the credentials configured during the inital setup of Magento.
