---
meta:
  title: How to deploy ERPNext 13 on Ubuntu 20.04 LTS (Focal Fossa)
  description: Discover how to deploy the ERPNext 13 application on Ubuntu 20.04 LTS (Focal Fossa)
content:
  h1: How to deploy ERPNext 13 on Ubuntu 20.04 LTS (Focal Fossa)
...
This tutorial will show you how to deploy ERPNext 13 on a Scaleway machine running Ubuntu Focal Fossa (20.04 LTS). This could be a [Dedibox](https://www.scaleway.com/en/dedibox/), an [Instance](https://www.scaleway.com/en/virtual-instances/) or a [Elastic Metal server](https://www.scaleway.com/en/elastic-metal/).
...
3. Create a new user. Some of the commands we will be running later in this tutorial should not be carried out as root. Here, we call the new user `mariecurie`, but you could call them `erpnext` or whatever you like. Enter a password for them when prompted. You can leave the user information at default values if you wish.
  ```
  adduser mariecurie
  ```
...
11. Return back into the `/home` directory of the user:
...
1. Open a web browser on your local computer and point it to `https://your_erpnext_domain/`. The login page displays. Enter the user name **Administrator** in the email address box, and the password that you defined during the setup. Then click **Login**:
