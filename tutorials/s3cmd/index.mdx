...
content:
  h1: How to use Object Storage with s3cmd
...
2. Open the file with a text editor, e.g. nano:
  ...
  nano .s3cfg
  ```
3. Copy and paste the following text into the file, then save and close. Make sure you use the `host-base`, `host-bucket` and `bucket_location` details of your own bucket (visible in the **Bucket Settings** tab of the console). You will also need to replace `<ACCESS_KEY>` and `<SECRET_KEY>` with your [own API credentials](/console/my-project/how-to/generate-api-key/).
  ```bash
  [default]
...
You can configure s3cmd to use different configuration files on the same computer to manage buckets in multiple regions. Here, we assume that your default configuation file was for a bucket in `nl-ams`, and we create a new configuration file for buckets in `fr-par`.
...
2. Create a new file named `.s3cfg-fr-par` in your home directory.
  ...
  touch .s3cfg-fr-par
  ```
3. Copy and paste the following text into the file, then save and close. Make sure you replace `<ACCESS_KEY>` and `<SECRET_KEY>` with your [own API credentials](/console/my-project/how-to/generate-api-key/). 
  ```bash
  [default]
  signature = s3v4
