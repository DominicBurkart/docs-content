...
meta:
  ...
  description: This page explains how to setup a mailserver on Ubuntu Bionic Beaver.
content:
  h1: Setting up a Secure Mail Server
  paragraph: This page explains how to setup a mailserver on Ubuntu Bionic Beaver.
categories:
  - compute
  - instances
tags: security mailserver DKIM Rspamd MariaDB Roundcube
dates:
  validation: 2022-09-22
  posted: 2020-06-04
---
...
* To ensure that other servers will accept mails sent from your Instance a [valid reverse DNS](https://www.scaleway.com/en/docs/how-to-configure-reverse-dns/) within your own domain name (for example `mail.domain.com`) must be configured.
* The [SMTP ports](https://www.scaleway.com/en/faq/why-can-i-not-send-any-email/) have been unlocked in the security group of the server.
...
1. Set a root password for MariaDB:
    ...
    * `Set root password? [Y/n]` _Type Y_
    * `New password:` _Enter the password for the root user_
    * `Re-enter new password:` _Repeat the password_
    * `Remove anonymous users? [Y/n]` _Type Y_
    * `Disallow root login remotely? [Y/n]` _Type Y_
    * `Remove test database and access to it? [Y/n]` _Type Y_
    * `Reload privilege tables now? [Y/n]` _Type Y_
...
9. Enter the email address of the admin, and answer the questions of the CLI.
    ...
    Certbot will automatically request the certificate and rewrite the Nginx configuration file. The file `/etc/nginx/sites-enabled/mail.example.com.conf` should resemble the following example:
