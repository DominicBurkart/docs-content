...
The modular design of [Odoo](https://www.odoo.com/) allows developers to create additional modules or apps and distribute them through the Odoo marketplace, which provides more than 20,000 modules. This makes Odoo a versatile and customizable solution that can be adapted to many usage scenarios.

This tutorial will show you how to deploy Odoo 14 on a Scaleway machine running Ubuntu Focal Fossa (20.04 LTS). This could be a [Dedibox](https://www.scaleway.com/en/dedibox/), an [Instance](https://www.scaleway.com/en/virtual-instances/) or an [Elastic Metal server](https://www.scaleway.com/en/elastic-metal/).
...
4. Install the Python dependencies for Odoo:
  ```
  apt install git fontconfig fontconfig-config fonts-dejavu-core libfontconfig1 libjpeg-turbo8 libxrender1 build-essential python3 python3-pip python3-setuptools python3-dev python3-venv python3-wheel libxslt1-dev libzip-dev libldap2-dev libsasl2-dev node-less libjpeg-dev gdebi -y
  ```
