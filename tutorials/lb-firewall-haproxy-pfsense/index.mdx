---
meta:
  title: How to Build a Redundant Loadbalancer and Firewall Infrastructure With pfSense and HAproxy on a Scaleway Dedibox
  description: This page explains how to install pfSense on a Scaleway Dedibox dedicated server
content:
  h1: How to Build a Redundant Loadbalancer and Firewall Infrastructure With pfSense and HAproxy on a Scaleway Dedibox
...
<Message type="requirement">
  ...
  -  You have [created an Elastic Metal server](/compute/elastic-metal/how-to/create-server) or have a [Scaleway Dedibox](https://www.scaleway.com/en/dedibox/)
</Message>
...
[Scaleway Dedibox](https://www.scaleway.com/en/dedibox/) dedicated servers come with two physical network interfaces. In this configuration the interface `igb0` is configured as WAN (Internet) interface, and the interface `igb1` acts as LAN (RPNv2) interface.
With [RPNv2](https://blog.scaleway.com/2016/online-rpnv2-more-flexible-more-performant-more-reliable/), it is possible to configure multiple private networks on the physical network interface. We suppose the [RPNv2 group](https://documentation.online.net/en/dedicated-server/network/rpn-v2) is already configured.
...
8. On each firewall, edit the file `/etc/pfSense-devd.conf` by clicking on **Diagnostics** > **Edit File**. Enter the path `/etc/pfSense-devd.conf` in the form **Path to file to be edited** and click **Load**.
...
10. Create the `carpcontrol.sh` script by clicking on **Diagnostics** > **Edit file** on the first firewall. Enter the path `/root/carpcontrol.sh in placeholder.sh` in the form, paste the content and click **Save**.
  ...
  > `PF2_IPFO`: failover IP assigned on the ssecond firewall
