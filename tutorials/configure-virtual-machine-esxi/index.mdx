...
The process from this step forward may differ according to the KVM model being used. Please refer to the Dedibox [KVM over IP documentation](https://documentation.online.net/fr/dedicated-server/operating-system/custom-install/start) to conclude the installation.
...
The VMware ESXi hypervisor uses full virtualisation for virtual machines. They can be installed directly from an [ISO file](https://en.wikipedia.org/wiki/ISO_image), like a physical machine. To be able to launch the installation of virtual machines, ISO images have to be available in the local datastore.
