---
meta:
  title: Installing LEMP-Stack (Linux, Nginx, MySQL, PHP) on Ubuntu Focal Fossa
  description: How to install LEMP-Stack (Linux, Nginx, MySQL, PHP) on Ubuntu Focal Fossa (20.04)
content:
  h1: Installing LEMP-Stack (Linux, Nginx, MySQL, PHP) on Ubuntu Focal Fossa
  paragraph: How to install LEMP-Stack (Linux, Nginx, MySQL, PHP) on Ubuntu Focal Fossa (20.04)
tags: account LEMP-Stack Ubuntu-Focal Linux Nginx MySQL PHP certbot
categories:
  - compute
dates:
...
The [LEMP-Stack](https://lemp.io/) references a group of software that can be used to serve dynamic web applications and websites. **LEMP** is an acronym and stands for:
...
<Message type="requirement">
  ...
  - You have a [Scaleway Instance](/compute/instances/how-to/create-an-instance/) running on Ubuntu 20.04 (Focal Fossa)
...
Nginx stores the configuration of virtual hosts in server blocks. All available server blocks are located in the `/etc/nginx/sites-available/` directory.

1. Navigate to the relevant directory:
  ```
  cd /etc/nginx/sites-available/
  ```
2. Create a new server block configuration file and name it after the domain name that points to your Instance. Here we call it `example.com` and put the following content into the file:
  ```
    server {
          listen 80;
          root /var/www/html;
  ...
  Overview:
    ...
    - `location ~ \.php$` — The second location block handles the PHP processing by pointing Nginx to the fastcgi-php.conf configuration file and the php7.4-fpm.sock file, which declares what socket is associated with php-fpm.
