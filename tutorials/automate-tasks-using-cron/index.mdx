...
[Cron](https://manpages.ubuntu.com/manpages/focal/en/man8/cron.8.html) is daemon to execute scheduled commands on Unix-like operating systems, including Ubuntu Linux 20.04 LTS (Focal Fossa). 
The commands to be executed are stored in a table. This table is called Crontab, while the tasks executed by the Cron daemon are called Cronjobs. Crojobs are executed automatically, making cron useful for automating maintenance-related tasks.
